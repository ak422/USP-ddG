...
def per_directions(pos_atoms):
    R = construct_3d_basis(pos_atoms[:, :, BBHeavyAtom.CA], pos_atoms[:, :, BBHeavyAtom.C],
                           pos_atoms[:, :, BBHeavyAtom.N])

    pos_N = pos_atoms[:, :, BBHeavyAtom.N]  # (N, L, 3)
    pos_CA = pos_atoms[:, :, BBHeavyAtom.CA]
    pos_C = pos_atoms[:, :, BBHeavyAtom.C]
    pos_O = pos_atoms[:, :, BBHeavyAtom.O]

    dire_C = global_to_local(R, pos_CA, pos_C)
    dire_N = global_to_local(R, pos_CA, pos_N)
    dire_O = global_to_local(R, pos_CA, pos_O)

    per_direct_feat = torch.cat([dire_C, dire_N, dire_O], dim=-1)
    return per_direct_feat


def pairwise_directions(pos_atoms):
    R = construct_3d_basis(pos_atoms[:, :, BBHeavyAtom.CA], pos_atoms[:, :, BBHeavyAtom.C],
                           pos_atoms[:, :, BBHeavyAtom.N])
    ...
    N, L = pos_atoms.shape[:2]
    dire_CA = global_to_local(R, pos_CA[:, :, None].expand(N, L, L, 3), pos_CA[:, None, :].expand(N, L, L, 3))
    dire_C = global_to_local(R, pos_CA[:, :, None].expand(N, L, L, 3), pos_C[:, None, :].expand(N, L, L, 3))
    dire_O = global_to_local(R, pos_CA[:, :, None].expand(N, L, L, 3), pos_O[:, None, :].expand(N, L, L, 3))
    dire_N = global_to_local(R, pos_CA[:, :, None].expand(N, L, L, 3), pos_N[:, None, :].expand(N, L, L, 3))

    pair_direct_feat = torch.cat([dire_CA, dire_C, dire_O, dire_N], dim=-1)
    return pair_direct_feat
