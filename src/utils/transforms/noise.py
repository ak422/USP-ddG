...
from src.utils.transforms.select_atom import chi_atom15_index_map,atom15to37_index_map
from src.utils.protein.dihedral_chi import  atom37_to_torsion_angles, AA_TO_INDEX, ALL_ATOMS, ALL_ATOM_POSNS,ONE_TO_THREE, CHI_PI_PERIODIC_LIST
...
class AddAtomNoise(object):
    ...
    def __init__(self, noise_backbone=0.15, noise_sidechain=0.2):
        super().__init__()
        self.noise_backbone = noise_backbone
        self.noise_sidechain = noise_sidechain
    ...
    def __call__(self, data, dummy=None):
        ...
        protein["pos_heavyatom_ori"] = protein["pos_heavyatom"].clone()

        _get_noise = lambda: ((torch.randn_like(protein["pos_heavyatom"][:, 0:4, :]) * self.noise_backbone) * protein["mask_heavyatom"][:, 0:4, None])
        protein["pos_heavyatom"][:,0:4,:] = protein["pos_heavyatom"][:,0:4,:] + _get_noise()
        _get_noise = lambda: ((torch.randn_like(protein["pos_heavyatom"][:, 4:, :]) * self.noise_sidechain) * protein["mask_heavyatom"][:, 4:, None])
        protein["pos_heavyatom"][:,4:,:] = protein["pos_heavyatom"][:,4:,:] + _get_noise()
